...
void QNChatMessage::setText(QString text, QString SecsSinceUnixEpoch, QSize allSize, QNChatMessage::User_Type userType)
{
    m_msg = text;
    m_userType = userType;
    m_time = SecsSinceUnixEpoch;
    m_curTime = QDateTime::fromMSecsSinceEpoch(SecsSinceUnixEpoch.toInt()).toString("hh:mm");
    m_allSize = allSize;
    if(userType == User_Me) {
        if(!m_isSending) {
